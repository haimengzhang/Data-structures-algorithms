...
public class HashMapTest {
    ...
    public void testResize() {
        HashMap<String, String> h = new HashMap<String, String>(2); //2
        assertEquals(h.capacity(), 2); //2
        h.put("key", "value");
        assertEquals("value", h.get("key"));
        h.put("key2", "value2");
        assertEquals("value2", h.get("key2"));

        assertEquals(4, h.capacity()); //4

        h = new HashMap<String, String>(10, 1);//10
        for (int i = 1; i <= 10; i += 1) {
            h.put(Integer.toString(i), Integer.toString(i));
        }
        assertEquals(10, h.size());
        assertEquals(10, h.capacity()); //10
        h.put("11", "11"); // insert 11
        for (int i = 1; i <= 11; i++) {
            System.out.print(h.get(Integer.toString(i)));
        }
        assertEquals(11, h.size());
        assertEquals("11", h.get("11")); //
        assertEquals(20, h.capacity()); // 20
        assertEquals("10", h.get("10")); // lost 11 after resize
        h.remove("11");
